...
class NeRF(nn.Module):
    ...
    def __init__(
        ...
        if self.opts["decoup_nerf"]:
            # raise NotImplementedError
            self.opts["rgb_dirfree"] = True
            self.opts["rgb_timefree"] = True

        if self.opts["low_pos_emb"]:
            num_freq_xyz = 6
        ...
        self.basefield = CondMLP(
            ...
            D=5 if not self.opts["decoup_nerf"] else 2,
            W=W,
        ...
        self.colorfield = CondMLP(
            num_inst=self.num_inst,
            D=2 if not self.opts["decoup_nerf"] else 3,
            W=W,
