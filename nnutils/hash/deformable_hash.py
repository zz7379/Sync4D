...
from lab4d.nnutils.hash.feature_hash import FeatureNeRF
...
class DeformableHash(FeatureNeRF):
    ...
    def __init__(
        ...
        self.warp = SkinningWarp(frame_info, D=1)
        if self.opts["rac_no_template"]:
            self.warp.articulation.rest_joints *= 0.1
    ...
    def get_samples(self, Kinv, batch):
        ...
        if isinstance(self.warp, SkinningWarp):
            ...
            if Kinv.shape[0] == 4 and frame_id[0] == frame_id[-1]:
                pass
                (
                    t_articulation,
                    rest_articulation,
                ) = self.warp.articulation.get_vals_and_mean(frame_id[:1])
                samples_dict["t_articulation"] = (
                    t_articulation[0].repeat(inst_id.shape[0], 1, 1),
                    t_articulation[1].repeat(inst_id.shape[0], 1, 1)
                )
                samples_dict["rest_articulation"] = (
                    rest_articulation[0].repeat(inst_id.shape[0], 1, 1),
                    rest_articulation[1].repeat(inst_id.shape[0], 1, 1)
                )
                # import ipdb; ipdb.set_trace()
