...
def sample_cam_rays(hxy, Kinv, near_far, n_depth=64, depth=None, perturb=False):
    ...
    # 如果这里出问题，检查def get_samples(self, batch):
    try:
...
def render_pixel(field_dict, deltas, render_uncert=False):
    ...
    if render_uncert:
        log_w = torch.log(weights + 1e-6)
        entropy = -torch.sum(weights * log_w, -1, keepdim=True) / 8
        print("entropy max",entropy.max())
        entropy[entropy == 0] = 1
        rendered["entropy"] = entropy
        # 
        # for j in range(weights.shape[1]):
        #     if entropy[0,j,0] < 0.5 and entropy[0,j,0] > 0.1:
        #         import matplotlib.pyplot as plt
        #         import numpy as np
        #         # Define the points
        #         x = np.array([i for i in range(weights.shape[2])])
        #         y = np.array(weights[0,j,:].cpu().numpy())
        ...
        # save rendered["entropy"] and rendered["rgb"] to image
